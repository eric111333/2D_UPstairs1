...
public class Gound : MonoBehaviour
{
    ...
    private Vector3 groundPostion5;
    ...
    public void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (groundPostion2.x - groundPostion1.x < 0.5f && groundPostion2.x - groundPostion1.x > -0.5f)
            if (groundPostion2.x <= 0)
                groundPostion2.x++;
            else
                groundPostion2.x--;
        Instantiate(gound0, groundPostion2, Quaternion.identity);
        groundPostion5.y = 6.5f;
        groundPostion5.y += yGround;
        groundPostion5.x = Random.Range(-1.8f, 1.8f);
        if (groundPostion5.x - groundPostion2.x < 0.5f && groundPostion5.x - groundPostion2.x > -0.5f)
            if (groundPostion5.x <= 0)
                groundPostion5.x++;
            else
                groundPostion5.x--;
        Instantiate(gound0, groundPostion5, Quaternion.identity);
